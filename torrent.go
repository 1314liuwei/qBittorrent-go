...
func (c *Client) PauseTorrents(ctx context.Context, hashes ...string) error {
	...
	res, err := c.Get(ctx, "/api/v2/torrents/pause", map[string]interface{}{
		"hashes": hashStr,
	})
	if err != nil {
		return err
...
func (c *Client) DeleteTorrents(ctx context.Context, hashes ...string) error {
	for i := 0; i < len(hashes); i++ {
		if err := IsValidHash(hashes[i]); err != nil {
			return err
		}
	...
	hashStr := strings.Join(hashes, "|")
	res, err := c.Get(ctx, "/api/v2/torrents/delete", map[string]interface{}{
		"hashes": hashStr,
	})
	if err != nil {
		return err
...
func (c *Client) RecheckTorrents(ctx context.Context, hashes ...string) error {
	...
	hashStr := strings.Join(hashes, "|")
	res, err := c.Get(ctx, "/api/v2/torrents/recheck", map[string]interface{}{
		"hashes": hashStr,
	})
	if err != nil {
		return err
